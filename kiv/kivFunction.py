...
class BuiltInFunction(BaseFunction):
  ...
  def execute_in(self,exec_ctx):
    ...
    if not isinstance(object,(String,List)):
      return RuntimeResult().failure(RuntimeError(
          self.pos_start,
          self.pos_end,
          "in() can only be used with a list or string"
        ))
    if isinstance(object,List):
  ...
  execute_in.arg_names = ["object","elem"]
...
class ModuleOS(BaseFunction):
  ...
  def copy(self):
    copy = ModuleOS(self.name)
    copy.set_context(self.context)
    copy.set_pos(self.pos_start, self.pos_end)
    return copy

  def __repr__(self):
    return f"<module-os function {self.name}>"
  def execute_system_execute(self,exec_ctx):
    command = exec_ctx.symbol_table.get("command")
    os.system(command.value)
    return RuntimeResult().success(Int(0))
      
  execute_system_execute.arg_names = ["command"]
  
ModuleOS.symbols = {"system_execute":ModuleOS("system_execute"),"os_name":String(os.name)}

class ModuleRandom(BaseFunction):
  def __init__(self, name):
    super().__init__(name)
  def execute(self, args):
    res = RuntimeResult()
  ...
  def copy(self):
    copy = ModuleRandom(self.name)
    copy.set_context(self.context)
    copy.set_pos(self.pos_start, self.pos_end)
    return copy

  def __repr__(self):
    return f"<module-random function {self.name}>"
  def execute_random_int(self,exec_ctx):
    n1 = exec_ctx.symbol_table.get("n1")
    n2 = exec_ctx.symbol_table.get("n2")
    if (type(n1),type(n2)) != (Int,Int):
      return RuntimeResult().failure(TypeError(
        self.pos_start,
        self.pos_end,
        "params of random_int should be int but got " + type(n1).__name__ + " and for param 2 " + type(n2).__name__
      ))
    return RuntimeResult().success(Int(random.randint(n1.value,n2.value)))  
  execute_random_int.arg_names = ["n1","n2"]
  
ModuleRandom.symbols = {"random_int":ModuleRandom("random_int")}

class ModuleKeyboardMouse(BaseFunction):
  def __init__(self, name):
    super().__init__(name)
  def execute(self, args):
    res = RuntimeResult()
  ...
  def copy(self):
    copy = ModuleKeyboardMouse(self.name)
    copy.set_context(self.context)
    copy.set_pos(self.pos_start, self.pos_end)
    return copy

  def __repr__(self):
    return f"<module-KeyboardMouse function {self.name}>"
  def execute_press(self,exec_ctx):
    key = exec_ctx.symbol_table.get("key")
    pt.press(key.value)
    return RuntimeResult().success(Int(0))
      
  execute_press.arg_names = ["key"]
  def execute_moveMouse(self,exec_ctx):
    x = exec_ctx.symbol_table.get("x")
    y = exec_ctx.symbol_table.get("y")
    pt.moveTo(x.value,y.value)
    return RuntimeResult().success(Int(0))
      
  execute_moveMouse.arg_names = ["x","y"]
  def execute_write(self,exec_ctx):
    text = exec_ctx.symbol_table.get("text")
    interval = exec_ctx.symbol_table.get("interval")

    pt.write(text.value,interval=interval.value)
    return RuntimeResult().success(Int(0))
      
  execute_write.arg_names = ["text","interval"]
  def execute_leftClick(self,exec_ctx):
    pt.leftClick()
    return RuntimeResult().success(Int(0))
      
  def execute_doubleClick(self,exec_ctx):
    pt.doubleClick()
    return RuntimeResult().success(Int(0))
      
  execute_doubleClick.arg_names = []
  def execute_rightClick(self,exec_ctx):
    pt.rightClick()
    return RuntimeResult().success(Int(0))
      
  execute_rightClick.arg_names = []
  def execute_size(self,exec_ctx):
    pt.rightClick()
    return RuntimeResult().success(List( [ Int(pt.size()[0]), Int(pt.size()[1])]) )
      
  execute_size.arg_names = []
ModuleKeyboardMouse.symbols = {"size":ModuleKeyboardMouse("size"),"press":ModuleKeyboardMouse("press"),"moveMouse":ModuleKeyboardMouse("moveMouse"),"leftClick":ModuleKeyboardMouse("leftClick"),"doubleClick":ModuleKeyboardMouse("doubleClick"),"rightClick":ModuleKeyboardMouse("rightClick"),"write":ModuleKeyboardMouse("write")}
